...
export class LoanHelperService {
  ...
  async manageDreamPointCommitedAfterLoanCreation(
    createLoanDto: any,
  ): Promise<any> {
  ...
  async createCreditDisbursementTransaction(
    loan: Loan,
    disbursedLoanDto: DisbursedLoanDto,
  ): Promise<any> {
  ...
  async checkAndCreateWingTransferFeeTransaction(
    loan: Loan,
    disbursedLoanDto: DisbursedLoanDto,
  ): Promise<any> {
    // Verify this with Akash
    if (
      loan?.wire_transfer_type == this.globalService.WIRE_TRANSFER_TYPES.MOBILE
    ) {
      const disbursed_amount = loan.amount - loan.dream_point;
      const wing_wei_luy_transfer_fee =
        this.globalService.CALC_WING_WEI_LUY_TRANSFER_FEE(disbursed_amount);
      const transactionDto = {
        loan_id: loan.id,
        amount: wing_wei_luy_transfer_fee,
        image: disbursedLoanDto.image,
        type: this.globalService.TRANSACTION_TYPE
          .DEBIT_WING_WEI_LUY_TRANSFER_FEE,
        note: disbursedLoanDto.note,
        client_id: loan.client_id,
      };
  ...
  async updateLoanDataAfterDisbursement(
    loan: Loan,
    disbursedLoanDto: DisbursedLoanDto,
  ) {
    ...
    this.log.log(
      `Updating loan with data ${JSON.stringify(fields_to_be_update)}`,
    );
    return await this.loanRepository.update(loan.id, fields_to_be_update);
  }

  async updateZohoDataAfterDisbursement(
    loan: Loan,
    disbursedLoanDto: DisbursedLoanDto,
  ) {
    ...
    const zohoKeyValuePairs = {
      ...
      Payment_Status: new Choice(
        this.globalService.LOAN_PAYMENT_STATUS.PENDING,
      ),
    };
  ...
  async handleClientCreditRepayments(
    ...
    if (
      !loan ||
      !createRepaymentTransactionDto.amount ||
      loan.status != this.globalService.LOAN_STATUS.DISBURSED
    ) {
    ...
    if (createRepaymentTransactionDto.amount >= loan.outstanding_amount) {
      ...
      const extra_amount =
        +createRepaymentTransactionDto.amount - +loan.outstanding_amount;
      await this.createPartialTransactionOnFullyPaid(
        loan,
        createRepaymentTransactionDto,
      );
      await this.createCreditRepaymentTransaction(
        loan,
        createRepaymentTransactionDto,
      );
      await this.createFeePaymentTransaction(
        loan,
        createRepaymentTransactionDto,
      );
      await this.checkAndCreateCreditWingTransferFeeTransaction(
        loan,
        createRepaymentTransactionDto,
      );
      await this.checkAndCreateLateFeeTransaction(
        loan,
        createRepaymentTransactionDto,
      );
      await this.createDreamPointEarnedTransaction(
        loan,
        createRepaymentTransactionDto,
        extra_amount,
      );
      ...
      await this.updateClientAfterFullyPaid(
        loan,
        createRepaymentTransactionDto,
        extra_amount,
      );
      ...
      zohoKeyValuePairs = {
        ...
        Days_Fully_Paid: differenceInCalendarDays(
          new Date(),
          new Date(loan.disbursed_date),
        ),
      ...
      zohoKeyValuePairs = {
        Loan_Status: new Choice(
          this.globalService.ZOHO_LOAN_STATUS.PARTIAL_PAID,
        ),
        Last_Repaid_Date: new Date(),
      };
    }

    zohoKeyValuePairs.Paid_Amount =
      await this.transactionService.getTotalPaidAmount(
        createRepaymentTransactionDto.loan_id,
      );
    zohoKeyValuePairs.Outstanding_Balance =
      loan.outstanding_amount - createRepaymentTransactionDto.amount;
    this.zohoLoanHelperService.updateZohoFields(
      loan.zoho_loan_id,
      zohoKeyValuePairs,
      this.globalService.ZOHO_MODULES.LOAN,
    );
  ...
  async createCreditRepaymentTransaction(
    loan: Loan,
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
  ): Promise<any> {
  ...
  async createFeePaymentTransaction(
    loan: Loan,
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
  ): Promise<any> {
    const transactionDto = {
  ...
  async checkAndCreateCreditWingTransferFeeTransaction(
    loan: Loan,
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
  ): Promise<any> {
    if (loan.wing_wei_luy_transfer_fee) {
      const transactionDto = {
        ...
        image: createRepaymentTransactionDto.image,
        type: this.globalService.TRANSACTION_TYPE
          .CREDIT_WING_WEI_LUY_TRANSFER_FEE,
        note: createRepaymentTransactionDto.note,
      };
      const transaction = await this.transactionService.create(transactionDto);
      return transaction;
  ...
  async checkAndCreateLateFeeTransaction(
    loan: Loan,
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
  ): Promise<any> {
    if (loan.late_fee) {
  ...
  async createDreamPointEarnedTransaction(
    loan: Loan,
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
    extra_amount: number,
  ): Promise<any> {
  ...
  async updateLoanAfterFullyPaid(
    loan: Loan,
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
  ): Promise<any> {
    const today = new Date(); // current time
    ...
    this.log.log(
      `Updating loan after fully paid with data ${JSON.stringify(
        fields_to_be_update,
      )}`,
    );
  ...
  async updateClientAfterFullyPaid(
    loan: Loan,
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
    extra_amount: number,
  ): Promise<any> {
    const client_new_tier = +loan?.client?.tier + 1;
    const dream_point_earned =
      loan?.client?.dream_points_earned + loan?.dream_point + extra_amount;
    const dream_point_committed =
      loan?.client?.dream_points_committed - loan?.dream_point;
  ...
  async doProcessPartialPayment(
    ...
    this.log.log(
      `Updating loan outstandingvamount partial  with data ${JSON.stringify(
        fields_to_be_update,
      )}`,
    );
    await this.loanRepository.update(loan.id, fields_to_be_update);
  ...
  async createPartialTransactionOnFullyPaid(
    loan: Loan,
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
  ): Promise<any> {
    // Create transaction for partial payment
  ...
  async handleDreamPointRefundRepayments(
    createRepaymentTransactionDto: CreateRepaymentTransactionDto,
  ): Promise<any> {
    ...
    if (dream_points_earned < createRepaymentTransactionDto.amount) {
      ...
      throw new BadRequestException(
        'Forbidden',
        'Amount is Greater then Dream Point Balance.',
      );
    }
    ...
    const updateClientDto = {
      ...
      tier: 1,
      dream_points_earned:
        dream_points_earned - createRepaymentTransactionDto.amount,
    };
    this.eventEmitter.emit('client.update', updateClientDto);
    return response;
  }
  ...
  async updateSendpulseFieldsAsPerClientTier(
    loan: Loan,
    tier: number,
  ): Promise<any> {
    const new_tier = tier;
  ...
  calcLoanPaymentStatus(loan: Loan): string {
    const first_repayment_date =
      loan.previous_repayment_dates && loan.previous_repayment_dates.length
        ? loan.previous_repayment_dates[0]
        : loan.repayment_date;
    const grace_repayment_date = addDays(
      new Date(first_repayment_date),
      this.globalService.LOAN_GRACE_PERIOD_DAYS,
    );
