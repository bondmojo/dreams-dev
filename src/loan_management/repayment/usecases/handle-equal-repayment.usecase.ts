...
export class HandleEqualRepaymentUsecase extends HandleRepaymentUsecase {
  ...
  async process(
    processRepaymentDto: ProcessRepaymentDto,
    doCreatePartialPaymentTransaction = true,
    isFunctionCalledFromOverPaymentUsecase = false,
  ): Promise<any> {
    const loan = await this.loanService.findOneForInternalUse({
      id: processRepaymentDto.loan_id,
    });
    const scheudle_instalment = await this.repaymentScheduleService.findOne({
      loan_id: loan.id,
      scheduling_status:
    ...
    await this.createTransactions(
      ...
      scheudle_instalment,
      loan,
      doCreatePartialPaymentTransaction,
    );
    ...
    /**
     *  Do below tasks when function is not called from over payment usecase.
    ...
    if (!isFunctionCalledFromOverPaymentUsecase) {
      ...
      await this.updateIsInstalmentFullyPaidOnSendpulse(
        loan,
        isInstalmentFullyPaid,
      );
  ...
  async updateLoan(processRepaymentDto: ProcessRepaymentDto, loan: Loan) {
    ...
    const outstanding_amount =
      loan.outstanding_amount - processRepaymentDto.amount;
    ...
    if (outstanding_amount == 0) {
      // if outstanding balance is zero then consider as today is fully paid date
      zohoKeyValuePairs.Days_Fully_Paid = differenceInCalendarDays(
        new Date(),
        new Date(loan.disbursed_date),
      );
      zohoKeyValuePairs.Loan_Status = new Choice(
        this.globalService.ZOHO_LOAN_STATUS.FULLY_PAID,
      );
    }
    zohoKeyValuePairs.Paid_Amount = await this.getLoanTotalPaidAmount(loan.id);

    this.logger.log(
      `Updating Loan On Zoho ${loan.zoho_loan_id} ${JSON.stringify(
        zohoKeyValuePairs,
      )} `,
    );
    await this.zohoRepaymentHelperService.updateZohoFields(
      loan.zoho_loan_id,
      zohoKeyValuePairs,
  ...
  async updateClientAfterFullyPaid(
    ...
    const dream_point_earned =
      loan?.client?.dream_points_earned + loan?.dream_point;
    let dream_point_committed =
      loan?.client?.dream_points_committed - loan?.dream_point;

    // Make dream point commited to zero if its values is negative
    dream_point_committed =
      dream_point_committed < 0 ? 0 : dream_point_committed;
  ...
  async scheduleNextInstalment(scheudle_instalment: any, loan_id: string) {
    ...
    const next_scheudle_instalment =
      await this.repaymentScheduleService.findOne({
        loan_id: loan_id,
        ins_number: next_ins_number,
        scheduling_status:
          this.globalService.INSTALMENT_SCHEDULING_STATUS.NOT_SCHEDULED,
      });
    ...
    updateRepaymentScheduleDto.scheduling_status =
      this.globalService.INSTALMENT_SCHEDULING_STATUS.SCHEDULED;
    await this.repaymentScheduleService.update(updateRepaymentScheduleDto);

    let zohoKeyValuePairs: any = {};
    zohoKeyValuePairs = {
      Installment_Status: new Choice(
    ...
    await this.zohoRepaymentHelperService.updateZohoFields(
      next_scheudle_instalment.zoho_repayment_schedule_id,
      zohoKeyValuePairs,
      this.globalService.ZOHO_MODULES.REPAYMENT_SCHEDULES,
    );
  }

  async updateRepaymentSchedule(
    scheudle_instalment: any,
    processRepaymentDto: ProcessRepaymentDto,
  ) {
    ...
    updateRepaymentScheduleDto.total_paid_amount =
      Number(scheudle_instalment.total_paid_amount) +
      Number(processRepaymentDto.amount);
    await this.repaymentScheduleService.update(updateRepaymentScheduleDto);

    let zohoKeyValuePairs: any = {};
    zohoKeyValuePairs = {
      Repayment_Status: new Choice(
        this.globalService.INSTALMENT_PAYMENT_STATUS_STR[
          updateRepaymentScheduleDto.repayment_status
        ],
      ),
      Installment_Status: new Choice(
        this.globalService.INSTALMENT_SCHEDULING_STATUS_STR[
    ...
    zohoKeyValuePairs.Paid_Amount = Number(
      updateRepaymentScheduleDto.total_paid_amount,
    );
    this.logger.log(
      `Updating Repayment Schedule On Zoho ${
        updateRepaymentScheduleDto.id
      } ${JSON.stringify(zohoKeyValuePairs)} ${
        this.globalService.ZOHO_MODULES.REPAYMENT_SCHEDULES
      }`,
    );
    await this.zohoRepaymentHelperService.updateZohoFields(
      scheudle_instalment.zoho_repayment_schedule_id,
      zohoKeyValuePairs,
      this.globalService.ZOHO_MODULES.REPAYMENT_SCHEDULES,
    );
  }
  ...
  calcInsPaymentStatus(scheudle_instalment: any): any {
    const curr_repayment_date = scheudle_instalment.due_date;
    const grace_repayment_date = addDays(
      new Date(curr_repayment_date),
  ...
  async createTransactions(
    processRepaymentDto: any,
    scheudle_instalment: any,
    loan: Loan,
    doCreatePartialPaymentTransaction: boolean,
  ) {
    ...
    if (
      scheudle_instalment.ins_additional_fee &&
      scheudle_instalment.ins_additional_fee > 0
    ) {
  ...
  async updateSendpulseFieldsAfterFullyPaid(loan: Loan): Promise<any> {
    ...
    const new_next_loan_amount =
      this.globalService.TIER_AMOUNT[client_tier + 1];
