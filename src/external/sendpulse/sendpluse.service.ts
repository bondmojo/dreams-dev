...
export class SendpluseService {
  ...
  async getContact(id: string): Promise<SendPulseContactDto> {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.get<SendPulseResponseDto<SendPulseContactDto>>(
      ...
      this.log.log(
        `Successfully retrieved user data from send pulse ${response.statusText}`,
      );

      return response.data.data;
    } catch (error) {
      this.log.error(
        `SENDPUSLE SERVICE: ERROR OCCURED WHILE RUNNING GetContact:  ${error}`,
      );
  ...
  async runFlowV2(model: RunFlowModel) {
    try {
      await this.checkAndGenerateToken();
      const response = await firstValueFrom(
        this.httpService.post<SendPulseResponseDto<SendPulseContactDto>>(
      ...
      this.log.log(
        `Successfully initiated flow ${model.flow_id} with response ${response.statusText}`,
      );
    } catch (error) {
      this.log.error(
        `SENDPUSLE SERVICE: ERROR OCCURED WHILE RUNNING runFlowV2:  ${error}`,
      );
    }
  }
  ...
  async runFlow(model: DreamerModel, flow: string): Promise<any> {
    try {
      ...
      this.log.log(
        `Successfully initiated flow ${flow} with response ${response.statusText}`,
      );
    } catch (error) {
      this.log.error(
        `SENDPUSLE SERVICE: ERROR OCCURED WHILE RUNNING runFlow:  ${error}`,
      );
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.post<SendPulseResponseDto<SendPulseContactDto>>(
          ...
          {
            contact_id: variableDto.contact_id,
            variable_id: variableDto.variable_id,
            variable_value: variableDto.variable_value,
          },
          {
            headers: { Authorization: 'Bearer ' + this.token },
          },
      ...
      this.log.error(
        `SENDPUSLE SERVICE: ERROR OCCURED WHILE RUNNING setVariable:  ${error}`,
      );
      throw error;
    }
  }

  async updateSendpulseVariable(
    setVariableRequestDto: SetVariableRequestDto,
  ): Promise<string> {
  ...
  async createClientId(client: Partial<Client>): Promise<string> {
    try {
      ...
      //Client ID Variable
      variableDto.variable_id =
        this.globalService.SENDPULSE_VARIABLE_ID.CLIENT_ID;
      ...
      this.log.error(
        `SENDPUSLE SERVICE: ERROR OCCURED WHILE RUNNING createClientId:  ${error}`,
      );
    }
  }
    ...
    try {
      ...
      switch (applStatus) {
        case this.APPLICATION_STATUS[0]:
          flowId =
            this.globalService.SENDPULSE_FLOW.APPLICATION_STATUS_FLOW_ID
              .Approved;
          break;
        ...
        case this.APPLICATION_STATUS[2]:
          ...
          transfertypeDto.variable_id =
            this.globalService.SENDPULSE_VARIABLE_ID.ACTIVE_LOAN_ID;
          ...
          flowId =
            this.globalService.SENDPULSE_FLOW.APPLICATION_STATUS_FLOW_ID
              .Disbursed;
          break;
        case this.APPLICATION_STATUS[3]:
          flowId =
            this.globalService.SENDPULSE_FLOW.APPLICATION_STATUS_FLOW_ID[
              'Not Qualified'
            ];
      ...
      this.log.error(
        `SENDPUSLE SERVICE: ERROR OCCURED WHILE RUNNING updateApplicationStatus:  ${error}`,
      );
    }
  }
  ...
  async checkAndGenerateToken(): Promise<string> {
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.post<SendPulseTokenDto>(
      ...
      this.access_token_expiry_time =
        Date.now() + response.data.expires_in * 1000;

      this.log.log(
        `Generated token from the sendpulse server which expires in ` +
          this.access_token_expiry_time,
      );
      return response.data.access_token;
    } catch (error) {
      this.log.error(
        `SENDPUSLE SERVICE: ERROR OCCURED WHILE RUNNING checkAndGenerateToken:  ${error}`,
      );
    }
  }
}
