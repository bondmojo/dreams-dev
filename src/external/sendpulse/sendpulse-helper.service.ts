...
export class SendpulseHelperService {
  ...
  calculateLoan(calculateLoanDto: CalculateLoanDto): CalculateLoanResultDto {
    ...
    try {
      ...
      let tenure = parseInt(calculateLoanDto.tenure);
      ...
      if (tenure_type != this.globalService.LOAN_TENURE_TYPE['MONTHLY']) {
        throw new DreamsException(
          DreamsCode.INVALID_DATA,
          'Tenure Type implementation required',
        );
      }
      result.maxTenure = this.globalService.CLACULATE_MAX_TENURE({
        amount,
        max_repayment_date: calculateLoanDto.max_repayment_date,
      });
      tenure = Number(result.maxTenure);
      result.fee = (
        this.globalService.INSTALMENT_MEMBERSHIP_FEE * tenure
      ).toString();
      result.payableAmount = (
        amount +
        Number(result.fee) +
        Number(wing_transfer_fee)
      ).toString();
  ...
  caclulateRepaymentSchedule(
    calculateRepaymentScheduleDto: CalculateRepaymentScheduleDto,
  ): RepaymentScheduleDto[] {
    try {
      let tenure = Number(calculateRepaymentScheduleDto.loan_tenure);
      const wing_wei_luy_transfer_fee =
        Number(calculateRepaymentScheduleDto.wing_wei_luy_transfer_fee) ?? 0;
      const loan_amount =
        Number(calculateRepaymentScheduleDto.loan_amount) +
        wing_wei_luy_transfer_fee;
      ...
      for (let i = 0; i < tenure; i++) {
        ...
        repaymentScheduleDto.ins_overdue_amount = Number(
          (
            principal_amount + this.globalService.INSTALMENT_MEMBERSHIP_FEE
          ).toFixed(2),
        );
        const now = new Date();
        repaymentScheduleDto.due_date = format(
          addMonths(now, repaymentScheduleDto.ins_number),
          'dd-MM-yyyy',
        );
      ...
      this.log.debug(
        'Repayment Schedule Plan =' + JSON.stringify(repaymentScheduleArray),
      );
      return repaymentScheduleArray;
    } catch (error) {
      this.log.error(`Error in Repayment Schedule Creation ${error}`);
      throw new DreamsException(
        DreamsCode.UNKNOWN_ERROR,
        `Error in Repayment Schedule Creation ${error}`,
      );
    }
  }
  ...
  calculate(calculateDto: CalculationDto): CalculationResultDto {
    switch (calculateDto.operation_type) {
      ...
      default:
        throw new DreamsException(
          DreamsCode.PAUSE_SENDPULSE_FLOW_ERROR,
          'CALCULATION FAILED',
        );
    }
  }
  ...
  getRepaymentScheduleMessage(
    ...
    if (ln == 'kh') {
      repaymentMsgTemplate =
        this.globalService.kh.getString('repayment_schedule');
    } else {
      repaymentMsgTemplate =
        this.globalService.en.getString('repayment_schedule');
    }
