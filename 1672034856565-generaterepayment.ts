...
export class generaterepayment1672034856565 implements MigrationInterface {
    name = 'generaterepayment1672034856565'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`loan_repayment_schedule\` (\`id\` varchar(255) NOT NULL, \`instalment_number\` int NOT NULL, \`loan_id\` varchar(255) NOT NULL, \`client_id\` varchar(255) NOT NULL, \`ins_overdue_amount\` decimal(10,2) NOT NULL, \`ins_principal_amount\` decimal(10,2) NOT NULL, \`ins_membership_fee\` decimal(10,2) NOT NULL DEFAULT '3.00', \`ins_additional_fee\` decimal(10,2) NOT NULL DEFAULT '0.00', \`total_paid_amount\` decimal(10,2) NOT NULL DEFAULT '0.00', \`repayment_status\` varchar(255) NOT NULL, \`grace_period\` int NOT NULL, \`number_of_penalties\` int UNSIGNED NOT NULL DEFAULT '0', \`previous_repayment_dates\` json NULL, \`ins_from_date\` date NOT NULL, \`ins_to_date\` date NOT NULL, \`due_date\` date NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`loans\` CHANGE \`payment_status\` \`payment_status\` varchar(255) NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`loans\` CHANGE \`payment_status\` \`payment_status\` varchar(255) NULL DEFAULT 'Pending'`);
